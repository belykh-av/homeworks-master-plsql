CREATE OR REPLACE PACKAGE BODY ut_payment_detail_api_pack is
  ...
  procedure insert_or_update_payment_detail is
    ...
    v_payment_detail_array t_payment_detail_array
      := t_payment_detail_array(
           t_payment_detail(payment_detail_api_pack.c_payment_detail_field_id_note, v_payment_detail_field_value_note),
           t_payment_detail(payment_detail_api_pack.c_payment_detail_field_id_ip, v_payment_detail_field_value_ip),
  ...
  begin
    ...
    ut_common_pack.create_default_payment(v_payment_detail_array);

    --Получить ID созданного платежа
    v_payment_id := ut_common_pack.get_default_payment_id();
    ...
    ut.expect(v_payment_detail_field_value_ip,
      ...
      ut_common_pack.get_payment_detail_field_value(v_payment_id, payment_detail_api_pack.c_payment_detail_field_id_ip));
  ...
  procedure direct_insert_payment_with_disabled_api is
    v_payment_id payment.payment_id%type := ut_common_pack.get_default_payment_id;
  begin
    --Setup. Надо сначала удалить деталь платежа, которую мы будем вставлять в тесте
    payment_detail_api_pack.delete_payment_detail(
      v_payment_id,
      t_number_array(payment_detail_api_pack.c_payment_detail_field_id_note));

    common_pack.disable_api;

    insert into payment_detail(payment_id, field_id, field_value)
    values (v_payment_id, payment_detail_api_pack.c_payment_detail_field_id_note, 'Test');
  ...
  begin
    ...
    where payment_id = v_payment_id and field_id = payment_detail_api_pack.c_payment_detail_field_id_note;
