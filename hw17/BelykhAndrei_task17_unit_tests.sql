...
--Тест блокировки платежа (выполняется в разных сессиях)
begin
  for r in (select min(payment_id) as payment_id
            from payment
            where status = 0) loop
    dbms_output.put_line('payment_id=' || r.payment_id);
    payment_api_pack.try_lock_payment(r.payment_id);
  end loop;
end;
...
declare
  ...
  begin
    ...
    for r in (select p.payment_id
              from payment p join payment_detail pd on pd.payment_id = p.payment_id
              where p.status = common_pack.c_status_created and rownum = 1) loop
